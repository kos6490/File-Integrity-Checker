    ...
    def firstScan(self):
        try:
            ...
            if scan_results:
                for item in scan_results:
                    if isinstance(item, tuple):
                        ...
                        if error == "Permission Denied":
                            print(
                                f"{RED}[실패] 파일을 읽을 권한이 없습니다! : {filePath}{END}"
                            )
                        elif error == "Error":
                            print(
                                f"{RED}[실패] 알 수 없는 오류가 발생했습니다! : {filePath}{END}"
                            )
                ...
                print(
                    f"{RED}[오류] {self.filePath} : 알 수 없는 오류가 발생했습니다.{END}\n"
                )
    ...
    def integrityCheck(self):
        try:
            ...
            new, change, same, delete, rename, error = compare_hashes(
                self.filePath, self.output_filePath, current_hashes
            )
            ...
            for path in rename:
                print(f"{MAGENTA}[이동/이름 변경된 파일] : {path}{END}")
            for path in error:
                print(f"{RED}[거부된 파일] : {path}{END}")
            print()
