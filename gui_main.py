...
class GuiApp:
    def __init__(self):
        self.win = (
            Tk()
        )  # StringVar 변수 초기화 전 Tk()를 호출하여 Root Window 먼저 생성
    ...
    def setGui(self):
        ...
        self.firstScanResult = Entry(
            self.win,
            textvariable=self.firstScan_result,
            width=55,
            relief="flat",
            font="맑은고딕 10",
            state="readonly",
        ...
        self.integrityCheckResult = Entry(
            self.win,
            textvariable=self.integrityCheck_result,
            width=55,
            relief="flat",
            font="맑은고딕 10",
            state="readonly",
    ...
    def show_check_result_window(self, results):
        ...
        try:
            ...
            text = Text(
                ...
                wrap="word",
                font="맑은고딕 10",
            )  # 새로운 창에 검사 결과 출력하기 위한 위젯 생성
            text.pack(side=LEFT, fill=BOTH, expand=True)

            text.tag_config(
                "new", foreground="green", font="맑은고딕 10"
            )  # 검사 결과 출력 색상 변경을 위한 태그 추가
            ...
            text.tag_config("rename", foreground="purple", font="맑은고딕 10")

            scrollbar.config(command=text.yview)

            check_results.transient(
                self.win
            ...
            for path in delete:
                ...
                text.see(END)
            for path in rename:
                text.insert(END, "[이동/이름 변경된 파일] : " + path + "\n", "rename")
                text.update_idletasks()
                text.see(END)
            for path in error:
            ...
            text.insert(END, f"이동/이름 변경된 파일 개수 : {len(rename)}\n")
